...
# check if NP after passive verb matches
def passive_second_np_ignore_case(pred_sentence, gold_sentence, src_sentence):
  ...
  if aux_idx_gold > 0 and aux_idx_pred > 0:
    ...
    if idx_pred and idx_gold > 0:
      if is_german: #remove determiner for german
          idx_pred += 1
          idx_gold += 1
      np_len = len(gold_words) - 1 - idx_gold
      # German gold sentence will have the verb after the second NP
...
# check if NP after passive verb matches
def passive_second_n(pred_sentence, gold_sentence, src_sentence):
  ...
  if aux_idx_gold > 0:
    ...
    if idx_pred and idx_gold > 0:
      ...
      if gold_second_np == pred_second_np:
        return 1
      elif idx_pred > 0 and pred_second_np.strip() != "" and  pred_second_np != pred_words[1].lower():
        #if idx_pred < 0 or pred_second_np.strip() == ""
        print(pred_sentence)
        print(gold_sentence)
        print(pred_second_np)
        print(gold_second_np)
        print(src_sentence)
        if src_sentence.replace(",", "").replace("  ", " ").lower() == pred_sentence.lower():
            print("IDENT")
        print("-------------------------")
    elif idx_gold >0:
        print(pred_sentence)
...
def passive_second_n_missing(pred_sentence, gold_sentence, src_sentence):
  ...
  if aux_idx_gold > 0 and aux_idx_pred > 0:
    ...
    if idx_pred and idx_gold > 0:
      ...
      #print(gold_second_np)
      #print("------------------")
      if pred_second_np.strip() == "":
        return 1
  return 0



def first_np_eq_second_np(pred_sentence, gold_sentence, src_sentence):
  # remove comma
  pred_sentence = pred_sentence.replace(",", "").replace("  ", " ")
  gold_sentence = gold_sentence.replace(",", "").replace("  ", " ")
  pred_words = pred_sentence.split()
  ...
  if idx > 0:
    pred_first_np = " ".join(pred_words[0:idx]).lower()
    if pred_first_np in " ".join(pred_words[idx:idx*2+3]).lower():
        #print(pred_sentence)
        #print(gold_sentence)
        return 1
...
def first_n_eq_second_n(pred_sentence, gold_sentence, src_sentence):
  # remove comma
  pred_sentence = pred_sentence.replace(",", "").replace("  ", " ")
  gold_sentence = gold_sentence.replace(",", "").replace("  ", " ")
  pred_words = pred_sentence.split()
  ...
  idx_pred = -1
  if idx > 0:
    for i, word in enumerate(pred_words[idx+1:]):
      idx2 = i + idx + 1
      if word in DETERMINERS:
        idx_pred = idx2
        break
    ...
    if idx_pred < 0:
        return 0
    pred_first_n = pred_words[1].lower()
    pred_second_n = pred_words[idx_pred+1].lower()
    if pred_first_n == pred_second_n:
        #print(pred_sentence)
        #print(gold_sentence)
        return 1
...
# check if there is extra material after the second NP
def passive_extra_material_end(pred_sentence, gold_sentence, src_sentence):
  ...
  if aux_idx_gold > 0 and aux_idx_pred > 0:
    ...
    if len(pred_words) > (aux_idx_pred + 1) and pred_words[aux_idx_pred + 1] in PASSIVE_PREPOSITIONS:
      idx_pred = aux_idx_gold + 2
    elif len(pred_words) > (aux_idx_pred + 2) and  pred_words[aux_idx_pred + 2] in PASSIVE_PREPOSITIONS:
      idx_pred = aux_idx_gold + 3
    ...
    if idx_pred and idx_gold > 0:
      ...
      if len(pred_words[idx_pred+np_len:]) <= len(gold_words[idx_gold+np_len:]):
        return 1
...
METRIC_FUNCTIONS = {
  ...
  "no_extra_material_end": passive_extra_material_end,
  "second_np_ignore_case": passive_second_np_ignore_case,
  "second_noun": passive_second_n,
  "second_noun_missing": passive_second_n_missing,
  "first_np_eq_second_np": first_np_eq_second_np,
  "first_n_eq_second_n": first_n_eq_second_n,
